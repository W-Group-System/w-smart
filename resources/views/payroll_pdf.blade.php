...
<body> 
    ...
    <table style='font-size:9px;' width='100%' border='1'>
        ...
        @endphp
            @foreach($payroll->informations as $key => $pay)
                <tr>
                    ...
                    <td class='text-right'>{{number_format($pay->net_pay < 0 ? 0 : $pay->net_pay, 2) }}</td>
        ...
        <tfoot>
            @php
                $netPayTotal = $payroll->informations->filter(function($pay) {
                    return $pay->net_pay > 0;
                })->sum('net_pay');
            @endphp
            <tr>
                <th colspan=28>
            ...
            <tr>
                ...
                <th class='text-right'>{{number_format($netPayTotal, 2) }}</th>
                <th>&nbsp;</th>
            </tr>
            <tr>
                <th class='text-right' colspan=3>
