...
<nav class="sidebar sidebar-offcanvas" id="sidebar" style="border-right: 2px solid #ccc;">
    ...
    <ul class="nav flex-column">
        <li class="nav-item">
            <a class="nav-link d-flex align-items-center {{ request()->is('/') || request()->is('home') ? 'active' : '' }}"
                href="{{ url('/') }}">
        ...
        @if($isSuperAdmin || in_array('Procurement', $userFeatures))
            <li class="nav-item">
                <a class="nav-link d-flex align-items-center dropdown-toggle" href="#" role="button"
                    data-bs-toggle="collapse" data-bs-target="#procurementSubmenu"
        ...
        @if($isSuperAdmin || in_array('Settings', $userFeatures))
            <li class="nav-item">
                <a class="nav-link d-flex align-items-center dropdown-toggle {{ request()->is('settings*') ? '' : 'collapsed' }}"
                    href="#" role="button" data-bs-toggle="collapse" data-bs-target="#settingsSubmenu"
                    aria-expanded="{{ request()->is('settings*') ? 'true' : 'false' }}" aria-controls="settingsSubmenu">
